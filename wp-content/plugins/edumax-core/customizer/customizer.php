...
if (!class_exists('THMC_Framework')):
	...
	class THMC_Framework
	{
		...
		public function customizer_scripts()
		{
			...
			// wp_enqueue_style( 'thmc-select2', get_template_directory_uri().'/customizer/assets/select2/css/select2.min.css' );
			wp_enqueue_style('thmc-select2',plugins_url('assets/select2/css/select2.min.css',__FILE__));
			wp_enqueue_style('thmc-customizer',plugins_url('assets/css/customizer.css',__FILE__));

			// Load all js
			wp_enqueue_script( 'jquery-ui-datepicker' );
			wp_enqueue_script( 'wp-color-picker' );
			wp_enqueue_script( 'thmc-select2', plugins_url().'/edumax-core/customizer/assets/select2/js/select2.min.js', array('jquery'), '1.0', true );


			wp_enqueue_script( 'thmc-rgba-colorpicker', plugins_url().'/edumax-core/customizer/assets/js/thmc-rgba-colorpicker.js', array('jquery', 'wp-color-picker'), '1.0', true );

			wp_enqueue_script( 'thmc-customizer', plugins_url().'/edumax-core/customizer/assets/js/customizer.js', array('jquery', 'jquery-ui-datepicker'), '1.0', true );

			wp_localize_script( 'thmc-customizer', 'thm_customizer', array(
				'ajax_url' => admin_url( 'admin-ajax.php' ),
				'import_success' => esc_html__('Success! Your theme data successfully imported. Page will be reloaded within 2 sec.', 'edumax-core'),
				'import_error' => esc_html__('Error! Your theme data importing failed.', 'edumax-core'),
				'file_error' => esc_html__('Error! Please upload a file.', 'edumax-core')
			) );
		}
...
class THM_Customize
{
	...
	public function import_export_ui( $wp_customize )
	{
		...
		$wp_customize->add_section( 'thm_import_export', array(
			'title'           => esc_html__( 'Import/Export', 'edumax-core' ),
			'description'     => esc_html__( 'Import Export Option Data', 'edumax-core' ),
			'priority'        => 1000,
		) );
