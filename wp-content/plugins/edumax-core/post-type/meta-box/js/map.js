(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$edumax = this.$container.find( '.rwmb-map-edumax' );
			this.edumax = this.$edumax[0];
		...
		initMapElements: function () {
			var defaultLoc = this.$edumax.data( 'default-loc' ),
				latLng;
			...
			this.map = new google.maps.Map( this.edumax, {
				center: latLng,
				zoom: 14,
				streetViewControl: 0,
			...
			$address.autocomplete( {
				source: function ( request, response ) {
					var options = {
						...
						'region': that.$edumax.data( 'region' )
					};
